...
class Rediska_PubSub_Channel extends Rediska_Options_RediskaInstance implements Iterator, ArrayAccess
{
        ...
        while (true) {
            ...
            foreach ($this->_connections as $connection) {
                ...
                try {
                    ...
                    if (!in_array($response->getChannel(),
                        $this->_subscriptions)
                    ) {
    ...
    protected function _execCommand($command, $channels)
    {
        ...
        foreach ($channelsByConnections as $connectionAlias => $channels) {
            ...
            foreach ($channels as $channel) {
                $execCommand[] = $this->getRediska()
                        ->getOption('namespace') . $channel;
            }
        ...
        while (!empty($channelsByConnections)) {
            foreach ($channelsByConnections as $connectionAlias => $channels) {
                ...
                foreach ($channels as $channel) {
                    ...
                    if (($command == self::SUBSCRIBE && $response instanceof Rediska_PubSub_Response_Subscribe) || ($command == self::UNSUBSCRIBE && $response instanceof Rediska_PubSub_Response_Unsubscribe)) {
                        $key = array_search($channel,
     ...
     */
    protected function _addMessageToBuffer(
        Rediska_PubSub_Response_Message $message
    ) {
    ...
    protected function _getResponseFromConnection(
        ...
        if ($this->getRediska()
                ->getOption('namespace') !== '' && strpos($channel,
                $this->getRediska()->getOption('namespace')) === 0
        ) {
            $channel = substr($channel,
                strlen($this->getRediska()->getOption('namespace')));
        }

        switch ($type) {
            case self::SUBSCRIBE:
                return new Rediska_PubSub_Response_Subscribe($connection,
                    $channel);

            case self::UNSUBSCRIBE:
                return new Rediska_PubSub_Response_Unsubscribe($connection,
                    $channel);

            case self::MESSAGE:
                $message = $this->getRediska()
                    ->getSerializer()
                    ->unserialize($body);

                return new Rediska_PubSub_Response_Message($connection,
                    $channel, $message);
    ...
    protected function _throwIfNotSupported()
    {
        ...
        if (version_compare($version,
                $this->getRediska()->getOption('redisVersion')) == 1
        ) {
