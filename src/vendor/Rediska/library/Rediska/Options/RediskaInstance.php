...
class Rediska_Options_RediskaInstance extends Rediska_Options
{
    ...
    public function getRediska()
    {
        if (!is_object($this->_rediska)) {
            $this->_rediska = self::getRediskaInstance($this->_rediska,
                $this->_optionsException, $this->_rediskaOptionName);
        }

        return $this->_rediska;
    }

    public static function getRediskaInstance(
        $rediska,
        $exceptionClassName = 'Rediska_Exception',
        $optionName = 'rediska'
    ) {
