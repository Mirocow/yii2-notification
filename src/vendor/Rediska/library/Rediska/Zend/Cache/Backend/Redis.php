...
class Rediska_Zend_Cache_Backend_Redis extends Zend_Cache_Backend implements Zend_Cache_Backend_ExtendedInterface
{
    ...
    public function getRediska()
    {
        if (!is_object($this->_rediska)) {
            $this->_rediska = Rediska_Options_RediskaInstance::getRediskaInstance($this->_rediska,
                'Zend_Cache_Exception', 'backend');
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        if ($lifetime) {
            $result = $this->getRediska()
                ->setAndExpire($id, array($data, time(), $lifetime), $lifetime);
        } else {
            $result = $this->getRediska()
                ->set($id, array($data, time(), $lifetime));
        }
    ...
    public function touch($id, $extraLifetime)
    {
        ...
        if (is_array($tmp)) {
            ...
            return $this->getRediska()
                ->setAndExpire($id, array($data, time(), $newLifetime),
                    $newLifetime);
        }
