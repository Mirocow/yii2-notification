...
class Rediska_Zend_Session_SaveHandler_Redis extends Rediska_Options_RediskaInstance implements Zend_Session_SaveHandler_Interface
{
    ...
    public function __construct($options = array())
    {
        ...
        if (!isset($options['lifetime'])) {
            ...
            if ($lifetime != 0) {
                ...
                trigger_error("Please set session.gc_maxlifetime to enable garbage collection.",
                    E_USER_WARNING);
            }
        }
    ...
    public function write($id, $data)
    {
        ...
        return $this->getRediska()
            ->setAndExpire($this->_getKeyName($id), $data,
    ...
    {
        try {
            return $this->_set->removeByScore(0,
                time() - $this->getOption('lifetime'));
        } catch (Rediska_Connection_Exec_Exception $e) {
            $this->_deleteSetOrThrowException($e);
        }
    }
     ...
     * @return void
     */
    protected function _deleteSetOrThrowException(
        Rediska_Connection_Exec_Exception $e
    ) {
