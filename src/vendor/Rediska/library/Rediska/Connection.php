...
class Rediska_Connection extends Rediska_Options
{
    ...
    public function connect()
    {
        if (!$this->isConnected()) {
            ...
            $this->_socket = @stream_socket_client($socketAddress, $errno,
                $errmsg, $this->getTimeout(), $flag);
            ...
            // Send password
            if ($this->getPassword() != '') {
                $auth = new Rediska_Connection_Exec($this,
            ...
            if ($this->_options['db'] !== self::DEFAULT_DB) {
                $selectDb = new Rediska_Connection_Exec($this,
                    "SELECT {$this->_options['db']}");
                try {
                    $selectDb->execute();
                } catch (Rediska_Command_Exception $e) {
    ...
    public function setBlockingMode($flag = true)
    {
        ...
        if ($this->isConnected()) {
            stream_set_blocking($this->_socket,
                $this->_options['blockingMode']);
        }
