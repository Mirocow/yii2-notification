...
class Rediska_Command_GetSortedSet extends Rediska_Command_Abstract
{
    ...
    public function parseResponse($response)
    {
        if ($this->responseIterator) {
            ...
            if ($this->withScores) {
                $values = Rediska_Command_Response_ValueAndScore::combine($this->_rediska,
                    $values);
            } else {
                $values = array_map(array(
                    $this->_rediska->getSerializer(),
                    'unserialize'
                ), $values);
            }
    ...
    public function parseIteratorResponse($response)
    {
        if ($this->withScores) {
            ...
            return new Rediska_Command_Response_ValueAndScore(array(
                'value' => $value,
                'score' => $score
            ));
        } else {
