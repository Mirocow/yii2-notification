...
class Rediska_Command_PopFromListBlocking extends Rediska_Command_Abstract
{
    ...
    public function create($keyOrKeys, $timeout = 0, $pushToKey = null)
    {
        ...
        if ($pushToKey !== null) {
            ...
            $this->_storeConnection = $this->getRediska()
                ->getConnectionByKeyName($pushToKey);
            ...
            foreach ($keysByConnections as $connectionAlias => $keys) {
                ...
                $execs[] = new Rediska_Connection_Exec($connections[$connectionAlias],
                    $command);
            }
    ...
    public function parseResponse($response)
    {
        if ($this->pushToKey !== null) {
            ...
            if (!$this->isAtomic()) {
                ...
                $exec = new Rediska_Connection_Exec($this->_storeConnection,
                    $command);
                $exec->execute();
            ...
            if (!is_array($this->keyOrKeys) && !empty($response)) {
                $result = $this->_rediska->getSerializer()
                    ->unserialize($response[1]);
            } else {
                $result = Rediska_Command_Response_ListNameAndValue::factory($this->_rediska,
                    $response);
            }
