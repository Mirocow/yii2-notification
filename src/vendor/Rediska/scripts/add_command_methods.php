...
{
    $command = file_get_contents(__DIR__ . '/../library/Rediska/Command/' . substr($class,
            16) . '.php');
    ...
    if (isset($matches[1])) {
        ...
        if ($classReflection->getParentClass()) {
            return getParams($classReflection,
...
foreach (Rediska_Commands::getList() as $name => $class) {
    ...
    // Rediska
    $createDocBlock = preg_replace('/@return .+/', "@return $return",
        $createDocBlock);
    $rediskaMethods .= "\n    $createDocBlock
    ...
    // Transaction
    $createDocBlock = preg_replace('/@return .+/',
        "@return Rediska_Transaction", $createDocBlock);
    $transactionMethods .= "\n    $createDocBlock
    public function $name($params { \$args = func_get_args(); return \$this->_addCommand('$name', \$args); }\n";

    // Pipeline
    $createDocBlock = preg_replace('/@return .+/', "@return Rediska_Pipeline",
        $createDocBlock);
    $pipelineMethods .= "\n    $createDocBlock
    public function $name($params { \$args = func_get_args(); return \$this->_addCommand('$name', \$args); }\n";
...
updateMethods(__DIR__ . '/../library/Rediska/Transaction.php',
    $transactionMethods);
updateMethods(__DIR__ . '/../library/Rediska/Pipeline.php', $pipelineMethods);
updateMethods(__DIR__ . '/../library/Rediska/Connection/Specified.php',
    $connectionMethods);
